		...
		<div class="col-xs-offset-4">			
			<c:choose>
				<c:when test="${login ne null}">
					...
					${login.id} 님환영합니다. <a href="/user/logout" class="btn btn-default">로그아웃</a>
					</div>					
				</c:when>
				<c:otherwise>
					<div class="pull-right">
					<a href="/user/login" class="btn btn-default">로그인</a>
					<a href="/user/join" class="btn btn-default">회원가입</a>
					</div>
				</c:otherwise>			
			</c:choose>
		</div>
		...
		<div class="row">
					...
					<tbody>
						<c:forEach items="${list}" var="boardVO">
							<tr>
								<td>${boardVO.bno}</td>
								<td><a
									href="/board/bread${pageMaker.makeQuery(pageMaker.cri.page)}&bno=${boardVO.bno}&orderType=${pageMaker.cri.orderType}">${boardVO.title}<strong>[${boardVO.replyCnt}]</strong></a></td>
			...
			<div class="row">
				<div class="text-center">
					...
					<a href="listCriteria?page=${pageMaker.cri.page-1}&perPageNum=${pageMaker.cri.perPageNum}">&laquo;</a></li>
					...
					<c:if test="${pageMaker.cri.page < pageMaker.totalCount}">
						<li><a
							href="listCriteria?page=${pageMaker.cri.page+1}&perPageNum=${pageMaker.cri.perPageNum}">&raquo;</a></li>
						...
						<li><a href="blistCriteria${pageMaker.makeQuery(1)}&orderType=${pageMaker.cri.orderType}">
						...
						<li
							...
							<a
							href="blistCriteria?page=${idx}&perPageNum=${pageMaker.cri.perPageNum}&orderType=${pageMaker.cri.orderType}">${idx}</a></li>
					</c:forEach>					
						...
						<%-- 	
							<c:choose>
								<c:when test="${pageMaker.cri.page >1}">
									<li><a href="blistCriteria${pageMaker.makeQuery(1)}">
											<span aria-hidden="true">First</span>
									</a></li>
									<li><a
										href="blistCriteria${pageMaker.makeQuery(pageMaker.cri.page-1)}">&laquo;</a></li>
								...
								<c:otherwise>
									<li class="disabled"><a
										href="blistCriteria${pageMaker.makeQuery(1)}"> <span
			...
			<div class="row">
				...
				<a class="btn btn-info pull-right"
					href="/board/create${pageMaker.makeQuery(pageMaker.cri.page)}&orderType=${pageMaker.cri.orderType}">글쓰기 <span class="fa fa-pencil-square-o fa-2x"></span></a>
				</div>
					...
					<span class="input-group-btn">
						...
						<a href="listCriteria" class="btn btn-warning"><span class="fa fa-list" aria-hidden="true"></span>&nbsp; 총 목록</a>
					</span>
				...
				//});
				 ...
				 $("#searchBtn").click(function () {
						...
						//	self.location="/board/listCriteria?page=${pageMaker.cri.page}&perPageNum="+ea+"&change=${true}&revPerPageNum2=${pageMaker.cri.perPageNum}";
							...
							var uri = "/sboard/blistCriteria${pageMaker.makeQuery(1)}"+"&searchType="+$("select#ssel option:selected").val()+"&keyword="+$("#keyword").val();			//상대좌표
			...
			function getServerTime() {
				$.ajax({
					...
					success: function(result) {
						...
						$("#serverTime").html("<span style=font-size:40px;> 전체목록  - 시간 : " + result + ".</span>");
