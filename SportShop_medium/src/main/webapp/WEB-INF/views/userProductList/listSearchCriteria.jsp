...
<script type="text/javascript">
	$(document).ready(function() {
		$("#sel").change(function(){	
			...
			self.location="listSearchCriteria?page=${pageMaker.cri.page}&perPageNum="+ea;
			
		});
		
		$("#searchBtn").click(function () {
		
			var uri = "listSearchCriteria${pageMaker.makeQuery(1)}"+"&searchType="+$("select#ssel option:selected").val()+"&keyword="+$("#keyword").val();
	...
	<div class="container">
		<div class="row">
			...
			<select class="selectpicker show-tick" data-width="auto"
				id="sel3" name="perPageNum">
				<option class="text-center" disabled
					data-icon="glyphicon glyphicon-list-alt"
					data-header="Select a condiment">정렬하기</option>
				<option class="text-center"
					${pageMaker.cri.perPageNum==12?'selected':'' } value="????">????</option>
				<option class="text-center"
					${pageMaker.cri.perPageNum==24?'selected':'' } value="????">????
					보기</option>
				<option class="text-center"
					${pageMaker.cri.perPageNum==48?'selected':'' } value="????">????
					보기</option>
				<option class="text-center"
					${pageMaker.cri.perPageNum==72?'selected':'' } value="????">????
					보기</option>
			</select>
			<select class="selectpicker show-tick" data-width="auto"
			...
			<ul id="proudctList" class="clearfix proudctList">
				...
				<li class="col-xs-3">
				<span>
					<a href="/userProductList/productDetail?pno=${pList.pno}"><img alt="메인이미지" src="/displayFile?fileName=${pList.p_img_1}"></a>
				</span>
					<div>
		...
		<div class="row">
				<div class="text-center">
					<ul class="pagination">
						...
						<li><a href="listSearchCriteria${pageMaker.makeSearch(1)}">
											<span aria-hidden="true">First</span>
									</a></li>
						<li><a
							href="listSearchCriteria${pageMaker.makeSearch(pageMaker.cri.page-1)}">&laquo;</a></li>
						...
						<li							
							...
							href="listSearchCriteria?page=${idx}&perPageNum=${pageMaker.cri.perPageNum}&searchType=${pageMaker.cri.searchType}&keyword=${pageMaker.cri.keyword}">${idx}</a></li>
					</c:forEach>					
					
					<c:if test="${pageMaker.cri.page < pageMaker.totalPage}">
						<li><a
							href="listSearchCriteria${pageMaker.makeSearch(pageMaker.cri.page+1)}">&raquo;</a></li>
							
						<li><a
										href="listSearchCriteria${pageMaker.makeSearch(pageMaker.totalPage)}">
			...
			<div class="row">
				...
				<!-- <i class="fa fa-pencil-square" aria-hidden="true"></i> -->
					...
					<span class="input-group-btn">
						...
						<a href="listSearchCriteria" class="btn btn-warning"><span class="fa fa-list" aria-hidden="true"></span>&nbsp; 총 상품 목록</a>
					</span>
